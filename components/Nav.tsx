...
const akashicGroups = [
  {
    ...
    items: [
      ...
      { title: "Akashic Data Warehouse", desc: "Centralized, secure storage for all your business records", href: slug("Akashic Data Warehouse") },
...
const solutionsGroups = [
  {
    ...
    items: [
      ...
      { title: "Akashic Knowledge", desc: "Centralized intelligence for your entire organisation", href: slug("Akashic Knowledge") },
    ] as MenuLink[],
  },
  {
    heading: "Sectors",
...
const deliveryGroups = [
  {
    ...
    items: [
      ...
      { title: "Sovereign Blueprint", desc: "Design secure, localized data infrastructure", href: slug("Sovereign Blueprint") },
  ...
  },
  {
    heading: "Engineer",
    items: [
      { title: "Platform Deployment", desc: "End-to-end implementation of the DHIRA platform", href: slug("Platform Deployment") },
      { title: "Legacy Modernization", desc: "Seamlessly upgrade from outdated legacy systems", href: slug("Legacy Modernization") },
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {item.desc && <div className="text-xs text-secondary-text">{item.desc}</div>}
      </div>
    </Link>
  );
}
...
export default function Nav() {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b border-subtle-stroke bg-white/95 backdrop-blur-md transition-transform duration-300 ease-settle ${showHeader ? "translate-y-0" : "-translate-y-full"}`}>
      <nav className="mx-auto w-full max-w-[1280px] px-4 sm:px-6 lg:px-8 flex h-20 items-center justify-between">
      ...
      {mobileOpen && (
        <div className="border-t border-subtle-stroke bg-white px-4 py-4 lg:hidden">
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-wide text-tertiary-text">Akashic</p>
            {akashicGroups.flatMap((group) => [
              <p key={`${group.heading}-h`} className="pt-1 px-2 text-[11px] uppercase tracking-wide text-tertiary-text">{group.heading}</p>,
            ...
            <p className="pt-2 text-xs font-medium uppercase tracking-wide text-tertiary-text">Solutions</p>
            {solutionsGroups.flatMap((group) => [
              <p key={`${group.heading}-h`} className="pt-1 px-2 text-[11px] uppercase tracking-wide text-tertiary-text">{group.heading}</p>,
              ...group.items.map((item) => (
                <Link key={item.title} href={item.href} className="block rounded-lg px-2 py-1.5 text-sm font-medium text-primary-text hover:bg-tertiary-bg">
                  {item.title}
            ...
            <p className="pt-2 text-xs font-medium uppercase tracking-wide text-tertiary-text">Delivery</p>
            ...
            <p className="pt-2 text-xs font-medium uppercase tracking-wide text-tertiary-text">Insights</p>
            {insightsItems.map((item) => (
              <Link key={item.title} href={item.href} className="block rounded-lg px-2 py-1.5 text-sm font-medium text-primary-text hover:bg-tertiary-bg">
                {item.title}
              </Link>
            ))}
            <p className="pt-2 text-xs font-medium uppercase tracking-wide text-tertiary-text">Company</p>
