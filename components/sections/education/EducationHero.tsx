...
/*
 * [00] Education Hero: real campus texture plus a simulated Akashic student
 * success desk. The console is storytelling UI only: hardcoded demo readouts,
 * signal states, and a converging-risk trace, not live product functionality
 * (§8a).
 */
...
const signals = [
  { code: "ATT", name: "Attendance", state: "ok" },
  { code: "LMS", name: "Scores", state: "ok" },
  { code: "FEE", name: "Fees", state: "risk" },
  { code: "—", name: "Outreach", state: "ok" },
] as const;

const readouts = [
  { label: "Signals", value: "3 of 3" },
  { label: "Flagged", value: "Wk 3" },
  { label: "Window", value: "1 term" },
];

const eduSystems = ["SIS", "LMS", "Assessment", "Attendance", "Fees", "Placement"];

const signalRows = [
  { label: "Learner signal", value: "Attendance · scores · fees" },
  { label: "Journey graph", value: "Enrolment · course · placement" },
  { label: "Institution record", value: "Cohort · programme · outcome" },
];
  ...
  return (
    <div className="px-4 pb-1 pt-3.5">
      ...
      <svg viewBox="0 0 440 74" className="mt-2 h-[74px] w-full" aria-hidden>
        ...
        {/* at-risk band */}
        <line x1="0" y1={risk} x2="440" y2={risk} stroke="#8DA2F0" strokeWidth="1" strokeDasharray="3 4" opacity="0.7" />
        <text x="2" y={risk - 4} fontFamily="monospace" fontSize="8" fill="#c9cfda">
          AT-RISK BAND
        </text>
        <line x1="0" y1={baseline} x2="440" y2={baseline} stroke="#ffffff" strokeWidth="1" opacity="0.09" />
        {/* three thin signals each sliding, converging into the band */}
        <polyline fill="none" stroke="#8a929e" strokeWidth="1.3" points="0,26 88,30 176,36 220,41" />
        <polyline fill="none" stroke="#8a929e" strokeWidth="1.3" points="0,33 88,36 176,42 220,46" />
        <polyline fill="none" stroke="#8a929e" strokeWidth="1.3" points="0,40 88,43 176,47 220,50" />
        {/* combined risk crossing into the band */}
        <polyline fill="none" stroke="#8DA2F0" strokeWidth="1.8" points="220,46 264,52 308,56 352,59 396,61 432,62" />
        <circle cx="396" cy="61" r="3.5" fill="#8DA2F0" />
  ...
  return (
    <div className="w-full max-w-[520px] overflow-hidden rounded-frame border border-white/[0.14] bg-[rgba(14,16,20,0.55)] text-left shadow-[0_30px_90px_-30px_rgba(0,0,0,0.8)] backdrop-blur-2xl">
      ...
      <div className="px-4 pb-4 pt-3">
        <div className="relative">
          ...
          <div className="grid grid-cols-4">
            {signals.map((signal) => (
              <div key={signal.code + signal.name} className="flex flex-col items-start">
                <span
                ...
                <span
                  className={`text-[11.5px] font-semibold tracking-tight ${
                    signal.state === "risk" ? "text-blue-subtle" : "text-white"
                  }`}
                >
                  {signal.name}
                </span>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-white/[0.03] px-4 py-2.5 font-mono text-[8.5px] uppercase tracking-[0.06em]">
        <span className="text-white/50">SIS · LMS · assessment unified</span>
        <span className="text-white/35">Every learner one record</span>
      </div>
    </div>
  );
}

function EduSystemsRail() {
  return (
          ...
          <div>
            ...
            <ScrollReveal delay={80}>
              <p className="font-mono text-[11px] uppercase tracking-eyebrow text-white/[0.62]">
                Akashic for education <span className="text-white/[0.35]">·</span> Institutions &amp; national programmes
              </p>
            </ScrollReveal>
            ...
            <ScrollReveal delay={220}>
              <p className="mt-7 max-w-[37em] text-lg leading-relaxed text-white/[0.72]">
                Akashic already runs inside India&rsquo;s national learning
                platform, connecting billions of learning interactions into
                decision-ready insight. The same foundation connects your
                campus: one governed journey from enrolment to placement.
              </p>
            </ScrollReveal>
