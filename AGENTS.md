...
| 05 | methodology | `sections/delivery/DeliveryMethodology.tsx` | `bg-white` | Discover/Design/Deliver/Transfer on one unbroken dot rail (horizontal desktop, vertical mobile) — the "no hand-offs" visual — then the Engagement Console: a simulated week-4 delivery dashboard (§8a; AkashicCardChrome header) with phase-tracker bars, sprint ledger (SHIPPED / IN REVIEW / IN PROGRESS chips), artefact register, and the go-live milestone. Complements [02]'s Gantt (plan) with the live view (delivery); all accents blue |
...
Seven sector pages. `/sectors/public-sector` is the original bespoke page (copy from the user's content script, July 2026; figures public record). The six industry pages (Jul 2026; reworked twice on 20 Jul — first for "production-ready" density/no-photos feedback, then rebuilt around the user's Vectr reference: "each industry has its map and players and data flows like that") are what the nav's Solutions → Sectors dropdown, the footer's Solutions column, and the home `EverySector` "Deploy for …" links point at. Each runs **six sections**: Hero → Problem [01] → **Map [02]** → Solution [03] → Outcomes [04] → Close [05]. Five render through the shared template in `components/sections/sectors/`, driven per-sector by `sectorContent.ts` (typed config; capability chips instead of invented figures, Rule 4); Manufacturing swaps in its own creative hero + problem files (`components/sections/manufacturing/`). Hero visuals are **bespoke simulated product boards per sector** (`SectorBoards.tsx`, §8a — shared BoardShell chrome, sector-specific moment; Manufacturing's own torque-trace line console).

**`SectorMap` [02] is every page's signature section** (the sector touchline is its H2): a Vectr-style pinned scrollytelling **industry map** — an isometric line-art scene of the sector's players standing over the wide flat **Akashic plate**, with dashed blue flow routes + travelling packet dots (`animateMotion`) lighting up group by group across four story steps (players → field streams → enterprise resolves → answers flow back + alert chip). Machinery: `sectors/map/SectorMapScene.tsx` (iso kit: grid ground, shadows, tower/block/stack/shed/unit/mast/homes/pylons/console primitives; computed-geometry inline attrs per Rule 8's exception; reuses global `dashmove`/`ps-pulse`), `sectors/map/mapData.ts` (six scenes on one shared composition skeleton — slots A–E + console — so routes stay constant while players/labels/steps/alerts differ per sector), `sectors/SectorMap.tsx` (client scroll-driver: 340vh track, sticky pane, steps rail; static scene + stacked steps below lg; `position: sticky` — do NOT add `overflow-hidden` to this section, it breaks the pin). The earlier `SectorReplay` wire-log section was retired when the map absorbed its story.

Every `SectorOutcomes` [04] carries the page's one soft blue band (Rule 5a); every close card carries `#talk-to-our-team`; solution-step module chips link to `/akashic` anchors.

| Page | Sections | Signature pieces |
|---|---|---|
| `/sectors/public-sector` | `public-sector/` — PublicSectorHero, PublicSectorGap [01], PublicSectorProven [02], PublicSectorMoments [03], PublicSectorWhy [04], PublicSectorChain [05], PublicSectorDeploy [06], PublicSectorClose [07] | Public-record missions board hero card; two-tier proof (4 flagship count-up panels + "also in production" registry ledger); **tender-schedule ledger** (REQ-01…REQ-06 compliance rows with status chips); environment tiles with SOVEREIGN DEFAULT marker. `PublicSectorMoments` [03] carries the page's one soft blue band (Rule 5a) |
| `/sectors/manufacturing` | `manufacturing/` — ManufacturingHero, ManufacturingClocks [01], then shared SectorMap [02], SectorSolution [03], SectorOutcomes [04], SectorClose [05] | Creative hero: **line console** (§8a: torque-trace SVG with in-spec grey → blue drift segment + 08:14 marker, station rail with drift node, drift alert, readouts); **Two Clocks** problem split (dark machine-clock plate vs light meeting-clock plate on a dashed "the gap" axis); plant map (machines/mast/MES/warehouse/ERP → boardroom, "Drift flagged · ST-03") |
| `/sectors/healthcare` · `/sectors/finance` · `/sectors/retail` · `/sectors/education` · `/sectors/energy` | shared `sectors/` template, content from `sectorContent.ts` + `map/mapData.ts` | Split hero with the sector's own product board (SectorBoards); scenario dossier ("How it plays out today", red indicator bar) + three fracture columns with ghost numerals; the sector's industry map (hospital campus / bank channels / store-to-DC chain / campus / grid with pylons+meters); Unify → Govern → Predict → Act rail with module chips; before/after ledger card on the blue band; dark closure card with sector-stack chips |
...
`EverySector` references `/sectors/manufacturing-hq.jpg` etc. via `next/image` (six `{sector}-hq.jpg` photos). These must exist in `public/sectors/`. Never add `<Image>` or `<img>` tags pointing to paths that do not exist in `public/`.

Photography provenance: every photo on the site is stock under a free commercial use licence.

- `public/proof/` (ProvenAtScale) — `learning.jpg`, `mobility.jpg`, `immunisation.jpg`. Keep these names non-identifying.
- `public/sectors/` (EverySector) — the `{sector}-hq.jpg` set.
