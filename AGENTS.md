...
`EverySector` references `/sectors/manufacturing-hq.jpg` etc. via `next/image` (six `{sector}-hq.jpg` Unsplash photos, added 20 Jul 2026 — currently the site's only photography; sector-page heroes use simulated boards instead). These must exist in `public/sectors/`. Never add `<Image>` or `<img>` tags pointing to paths that do not exist in `public/`.
